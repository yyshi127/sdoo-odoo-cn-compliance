...
| Auditable delivery package | Bundle, manifest, acceptance summary, status summary and sign-off packet carry deterministic file inventory, SHA-256 evidence and human sign-off action items. | `tools/run_cn_delivery_acceptance.py`, `tools/build_cn_delivery_bundle.py`, `tools/verify_cn_delivery_artifacts.py`, `tools/summarize_cn_delivery_status.py`, `tools/generate_cn_signoff_packet.py`. | Delivery evidence proves software-package integrity and records required human decisions; it does not prove the correctness of a customer's tax position. |
