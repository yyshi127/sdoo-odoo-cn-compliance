...
| --- | --- | --- | --- |
| Installable and upgradeable Odoo 19 plugin | Main China country pack and optional XBRL parser are packaged as standard Odoo addons with versioned migrations, and release evidence must include both install-mode and update-mode Odoo acceptance summaries. | `addons/sudo_country_pack_cn/__manifest__.py`, `addons/sudo_country_pack_cn/migrations/`, `addons/sudo_country_pack_cn_einvoice_xbrl/`, delivery bundle metadata, `dist/cn_delivery_acceptance_mNNN_remote.json`, `dist/cn_delivery_acceptance_mNNN_upgrade_remote.json`. | Production deployment still requires backup/restore proof and a target-environment deployment decision before touching a live database. |
| Auditable delivery package | Bundle, manifest, acceptance summary, status summary, objective completion audit, sign-off packet, version-aligned sign-off evidence draft, sign-off validation and production release-control checklist carry deterministic file inventory, SHA-256 evidence and human sign-off action items. | `tools/run_cn_delivery_acceptance.py`, `tools/build_cn_delivery_bundle.py`, `tools/verify_cn_delivery_artifacts.py`, `tools/summarize_cn_delivery_status.py`, `tools/audit_cn_objective_completion.py`, `tools/generate_cn_signoff_packet.py`, `tools/render_cn_signoff_evidence_template.py`, `tools/validate_cn_signoff_evidence.py`, `docs/CHINA_PRODUCTION_RELEASE_CONTROL.md`. | Delivery evidence proves software-package integrity and records required human decisions; it does not prove the correctness of a customer's tax position. |
...
- update-mode acceptance summary generated by `tools/run_cn_delivery_acceptance.py`
  without `--install` against an already installed runtime database;
...
- production sign-off evidence draft generated by `tools/render_cn_signoff_evidence_template.py` from the current packet, with the static sample template kept structurally aligned to the generated decision contract, then production sign-off evidence validation generated by `tools/validate_cn_signoff_evidence.py` when human approvals are complete, including blocked objective areas, the production blockers still addressed by missing or invalid human evidence, and an automated evidence item plus machine-readable check proving every production blocker is mapped to at least one human sign-off action;
- objective completion audit generated after valid production sign-off evidence must move every objective item to `evidence_ready` and clear completion blockers before the delivery can be claimed achieved;
- delivery status must reject production sign-off evidence whose version, source commit or preview URL is not bound to the current delivery, and must report all detected binding mismatches together so a stale or cross-environment sign-off file cannot accidentally complete the objective audit;
- delivery status must translate unresolved production blockers into required human action keys, owners and evidence references so reviewers can move from a blocked status directly to the sign-off evidence they need to complete;
- Odoo runtime test log with `0 failed / 0 errors` for the selected acceptance profile.
- Odoo update-mode runtime test log with `0 failed / 0 errors` for the selected
  acceptance profile.
- production release-control checklist covering backup, isolated restore,
  rollback trigger, rollback owner and post-deployment monitoring controls.
