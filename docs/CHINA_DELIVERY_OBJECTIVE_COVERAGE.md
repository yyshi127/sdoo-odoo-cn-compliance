...
| Auditable delivery package | Bundle, manifest, acceptance summary, status summary, sign-off packet, version-aligned sign-off evidence draft and sign-off validation carry deterministic file inventory, SHA-256 evidence and human sign-off action items. | `tools/run_cn_delivery_acceptance.py`, `tools/build_cn_delivery_bundle.py`, `tools/verify_cn_delivery_artifacts.py`, `tools/summarize_cn_delivery_status.py`, `tools/generate_cn_signoff_packet.py`, `tools/render_cn_signoff_evidence_template.py`, `tools/validate_cn_signoff_evidence.py`. | Delivery evidence proves software-package integrity and records required human decisions; it does not prove the correctness of a customer's tax position. |
...
- production sign-off evidence draft generated by `tools/render_cn_signoff_evidence_template.py` from the current packet, then production sign-off evidence validation generated by `tools/validate_cn_signoff_evidence.py` when human approvals are complete, including blocked objective areas when sign-off evidence is missing or invalid;
