...
| `production_deployment_decision` | Release owner | `deploy`, `deploy_with_limitations`, `defer`, `reject` | Completed `docs/CHINA_PRODUCTION_SIGNOFF_TEMPLATE.md` including delivery version, source commit, bundle SHA-256, manifest aggregate hash, target database, target company scope, backup/restore proof, rollback trigger and owner, deployment window, monitoring owner and limitation statement. |
