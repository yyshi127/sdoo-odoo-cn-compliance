...
The exported checklist includes an **Automated Review Prefill** section that
summarizes runtime acceptance, the representative company and period,
risk/remediation/report counts, controlled AI traceability, rule/source
governance and customer data coverage from the bound sign-off packet. Reviewers
should use this section to avoid copying machine evidence by hand. It is
deliberately unable to populate reviewer names, decisions, dates, evidence
references or notes and therefore never counts as a human approval.
