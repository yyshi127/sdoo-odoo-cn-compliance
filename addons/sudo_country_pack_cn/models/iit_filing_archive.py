...
_CN_IIT_ARCHIVE_LINK_MARKER = object()


class SudoComplianceFiling(models.Model):
    _inherit = "sudo.compliance.filing"

    cn_iit_reconciliation_run_id = fields.Many2one(
    ...
    cn_iit_filing_record_id = fields.Many2one(
        related="cn_iit_reconciliation_run_id.withholding_record_id",
        string="受控个人所得税扣缴申报记录",
        readonly=True,
        groups="sudo_global_finance.group_compliance_manager",
    )
    cn_iit_payment_record_ids = fields.Many2many(
        related="cn_iit_reconciliation_run_id.payment_record_ids",
        string="受控个人所得税缴退税记录",
        readonly=True,
        groups="sudo_global_finance.group_compliance_manager",
    )
    cn_iit_currency_id = fields.Many2one(
        related="cn_iit_reconciliation_run_id.currency_id",
        string="个人所得税档案币种",
        readonly=True,
    )
    cn_iit_settlement_kind = fields.Selection(
        [
            ("none", "无应补应退"),
            ("payable", "应补缴税"),
            ("refund", "应退税款"),
        ...
        string="个人所得税结算方向",
        compute="_compute_cn_iit_settlement_kind",
    )
    cn_iit_refund_date = fields.Date(string="个人所得税实际退库日期", tracking=True)
    cn_iit_refund_reference = fields.Char(string="个人所得税退库参考号", tracking=True)
    cn_iit_filing_payable_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.filing_payable_amount",
        string="来源应补个人所得税额",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_filing_refundable_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.filing_refundable_amount",
        string="来源应退个人所得税额",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_paid_principal_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.paid_principal_amount",
        string="个人所得税缴款成功本金",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_reversed_principal_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.reversed_principal_amount",
        string="个人所得税已冲正本金",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_effective_paid_principal_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.effective_paid_principal_amount",
        string="个人所得税有效缴款本金",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_refunded_principal_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.refunded_principal_amount",
        string="个人所得税已退库本金",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_interest_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.interest_amount",
        string="个人所得税来源利息合计",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )
    cn_iit_penalty_amount = fields.Monetary(
        related="cn_iit_reconciliation_run_id.penalty_amount",
        string="个人所得税来源滞纳金罚款合计",
        currency_field="cn_iit_currency_id",
        readonly=True,
    )

    _cn_iit_run_unique = models.Constraint(
        "unique(cn_iit_reconciliation_run_id)",
        "同一个人所得税勾稽批次只能关联一份申报缴退税档案。",
    )

    def write(self, values):
        changed = set(values)
        if "cn_iit_reconciliation_run_id" in changed and any(
            filing.state != "draft" for filing in self
        ) and self.env.context.get(
            "cn_iit_filing_archive_link"
        ) is not _CN_IIT_ARCHIVE_LINK_MARKER:
            raise UserError(
                _("进入准备流程后不能更换中国个人所得税勾稽来源。")
            )
        if changed & {"cn_iit_refund_date", "cn_iit_refund_reference"} and any(
            filing.cn_payment_checksum for filing in self
        ):
            raise AccessError(
                _("退库档案封存后不能修改退库日期或参考号。")
            )
    ...
    def _check_cn_iit_archive_scope(self):
        for filing in self:
            linked_runs = sum(
                bool(run)
                for run in (
                    filing.cn_vat_reconciliation_run_id,
                    filing.cn_cit_reconciliation_run_id,
                    filing.cn_iit_reconciliation_run_id,
                )
            )
            if linked_runs > 1:
                raise ValidationError(
                    _("同一申报档案不能同时关联多个中国税种勾稽来源。")
                )
            run = filing.cn_iit_reconciliation_run_id
            if not run:
                continue
            if filing.country_id.code != "CN":
                raise ValidationError(
                    _("中国个人所得税档案只能关联中国合规档案。")
                )
            if (
                run.profile_id != filing.profile_id
                or run.company_id != filing.company_id
            ):
                raise ValidationError(
                    _("个人所得税勾稽来源必须属于当前公司和合规档案。")
                )
            if (
                ...
                raise ValidationError(
                    _("个人所得税勾稽来源与申报档案期间必须完全一致。")
                )
            if (filing.filing_code or "").strip().upper() != "CN-IIT-WHT":
                raise ValidationError(
                    _("受控个人所得税档案的申报编码必须为 CN-IIT-WHT。")
                )
            if (
                (filing.filing_type or "").strip().lower()
                != "cn_iit_withholding_return"
            ):
                raise ValidationError(
                    _(
                        "受控个人所得税档案的申报类型必须为 "
            ...
            if (
                not filing.obligation_id
                or filing.obligation_id.code != "CN-IIT-WHT"
            ):
                raise ValidationError(
                    _("受控个人所得税档案必须关联 CN-IIT-WHT 适用义务。")
                )

    @api.depends(
        "cn_iit_reconciliation_run_id",
        "cn_iit_reconciliation_run_id.has_filing_payable_amount",
        "cn_iit_reconciliation_run_id.filing_payable_amount",
        "cn_iit_reconciliation_run_id.has_filing_refundable_amount",
        "cn_iit_reconciliation_run_id.filing_refundable_amount",
    )
    def _compute_cn_iit_settlement_kind(self):
        for filing in self:
            run = filing.cn_iit_reconciliation_run_id
            if not run:
                filing.cn_iit_settlement_kind = "none"
                continue
            if not (
                run.has_filing_payable_amount
                and run.has_filing_refundable_amount
            ):
                filing.cn_iit_settlement_kind = "unknown"
                continue
            payable = not run.currency_id.is_zero(run.filing_payable_amount)
            refundable = not run.currency_id.is_zero(
                run.filing_refundable_amount
            )
            if payable and refundable:
                filing.cn_iit_settlement_kind = "conflict"
            elif payable:
                filing.cn_iit_settlement_kind = "payable"
            elif refundable:
                filing.cn_iit_settlement_kind = "refund"
            else:
                filing.cn_iit_settlement_kind = "none"

    def _cn_iit_source_record(self, *, require_current):
        self.ensure_one()
        run = self.cn_iit_reconciliation_run_id
        if not run:
            raise UserError(
                _("请先关联中国个人所得税工资账表款勾稽结果。")
            )
        allowed_states = (
            {"succeeded"} if require_current else {"succeeded", "superseded"}
        )
        if run.state not in allowed_states:
            raise UserError(
                _("只有当前成功的个人所得税勾稽结果可以形成申报档案。")
            )
        if not run.result_checksum or not run.filing_snapshot_checksum:
            raise UserError(
                _("个人所得税勾稽结果缺少受控申报快照校验和。")
            )
        if run.result_integrity_state != "verified":
            raise UserError(_("个人所得税勾稽结果完整性校验失败。"))
        if run.filing_source_state != "available":
            raise UserError(
                _("个人所得税扣缴申报来源存在阻断，不能登记正式申报。")
            )
        record = run.withholding_record_id
        if not record:
            raise UserError(
                _("正式申报档案必须对应唯一受控个人所得税扣缴申报记录。")
            )
        if require_current and not record.is_current_result:
            raise UserError(
                _("受控个人所得税扣缴申报已被替代，请先重新执行期间勾稽。")
            )
        if record.dataset_id._current_integrity_state() != "verified":
            raise UserError(
                _("受控个人所得税扣缴申报源文件完整性校验失败。")
            )
        if record.quality_state == "error":
            raise UserError(
                _("受控个人所得税扣缴申报存在数据错误，不能登记正式申报。")
            )
        if record.return_status not in {"submitted", "accepted", "amended"}:
            raise UserError(
                _("受控个人所得税扣缴申报状态不是已申报、已受理或更正申报。")
            )
        if not record.submitted_at or not record.submission_reference:
            raise UserError(
                _("正式申报档案要求来源申报同时具有申报时间和参考号。")
            )
        if record.currency_id != run.currency_id:
            raise UserError(
                _("受控个人所得税扣缴申报币种与档案币种不一致。")
            )
        return record

    def _cn_iit_validate_obligation(self):
        self.ensure_one()
        obligation = self.obligation_id
        if not obligation or obligation.code != "CN-IIT-WHT":
            raise UserError(
                _("请先关联当前合规档案的 CN-IIT-WHT 适用义务。")
            )
        if obligation.applicability != "applicable":
            raise UserError(
                _("CN-IIT-WHT 义务尚未经过公司级适用性确认。")
            )
        if (
            not obligation.filing_required
            or obligation.filing_type != "cn_iit_withholding_return"
        ):
            raise UserError(
                _("CN-IIT-WHT 义务未配置为个人所得税扣缴申报义务。")
            )
        ...
        if (
            ...
            raise UserError(
                _("CN-IIT-WHT 义务的官方依据尚未有效复核或已经过期。")
            )
        return obligation

    def _cn_iit_validate_settlement_configuration(self):
        self.ensure_one()
        kind = self.cn_iit_settlement_kind
        if kind == "unknown":
            raise UserError(
                _("个人所得税来源未完整提供应补和应退字段，结算方向不能确认。")
            )
        if kind == "conflict":
            raise UserError(
                _("个人所得税来源同时存在正数应补和应退金额，请先复核来源。")
            )
        if kind == "payable" and not self.payment_required:
            raise UserError(
                _("当前来源存在应补个人所得税，请将申报档案标记为需要付款。")
            )
        if kind in {"none", "refund"} and self.payment_required:
            raise UserError(
                _("当前来源不存在应补个人所得税，不应将档案标记为需要付款。")
            )
        return kind

    @api.model
    def _cn_iit_amount_payload(self, currency, provided, amount):
        return {
    ...
    def _cn_submission_payload(self, record, evidence_records):
        self.ensure_one()
        if not self.cn_iit_reconciliation_run_id:
        ...
        return {
            "schema": "sdoo.cn.iit-withholding-filing-archive.v1",
            ...
            "reconciliation": {
                ...
                "tax_type_code": run.iit_tax_type_code,
            },
            "filing_record": {
                "record_id": record.id,
                ...
                "declared_line_count": (
                    record.declared_line_count
                    if record.has_declared_line_count
                    else None
                ),
                "amounts": {
                    field_name: self._cn_iit_amount_payload(
                        record.currency_id,
            ...
            "settlement_kind": self.cn_iit_settlement_kind,
    ...
    def _cn_payment_payload(self, evidence_records):
        ...
        kind = self.cn_iit_settlement_kind
        settlement_date = (
            self.payment_date if kind == "payable" else self.cn_iit_refund_date
        )
        settlement_reference = (
            self.payment_reference
            if kind == "payable"
            else self.cn_iit_refund_reference
        )
        return {
            "schema": "sdoo.cn.iit-withholding-settlement-archive.v1",
            "company_id": self.company_id.id,
            "profile_id": self.profile_id.id,
            "filing_id": self.id,
            "period_start": _date_string(self.period_start),
            ...
            "reconciliation": {
                ...
                "has_payable_payment_difference": (
                    run.has_payable_payment_difference
                ),
                ...
                "has_refundable_refund_difference": (
                    run.has_refundable_refund_difference
                ),
            ...
            "filing_settlement": {
                "payable": self._cn_iit_amount_payload(
                    run.currency_id,
                    run.has_filing_payable_amount,
                    run.filing_payable_amount,
                ),
                "refundable": self._cn_iit_amount_payload(
                    run.currency_id,
            ...
            "payment_records": [
                {
                    ...
                    "principal": self._cn_iit_amount_payload(
                        record.currency_id,
            ...
            "settlement_totals": {
                "paid_principal": str(
                    run.currency_id.round(run.paid_principal_amount)
                ),
    ...
    def _cn_iit_validate_submission(self):
        self.ensure_one()
        self._cn_iit_validate_obligation()
        self._cn_iit_validate_settlement_configuration()
        record = self._cn_iit_source_record(require_current=True)
        ...
        if not evidence:
            raise UserError(
                _("登记个人所得税申报前必须具有已验证的正式申报回执证据。")
            )
        return record, evidence

    def _cn_iit_validate_payment_records(self):
        self.ensure_one()
        run = self.cn_iit_reconciliation_run_id
        self._cn_iit_source_record(require_current=False)
        if run.result_integrity_state != "verified":
            raise UserError(_("个人所得税勾稽结果完整性校验失败。"))
        if (
            not run.payment_snapshot_checksum
            or run.payment_source_state != "available"
        ):
            raise UserError(
                _("个人所得税缴退税来源存在阻断，不能封存结算结果。")
            )
        records = run.payment_record_ids
        if not records:
            raise UserError(_("没有受控的个人所得税缴退税记录。"))
        for record in records:
            if record.dataset_id._current_integrity_state() != "verified":
                raise UserError(_("受控缴退税源文件完整性校验失败。"))
            if record.quality_state == "error":
                raise UserError(
                    _("受控缴退税记录存在数据错误，不能封存。")
                )
            if record.currency_id != run.currency_id:
                raise UserError(
                    _("受控缴退税记录币种与档案币种不一致。")
    ...
    def _cn_iit_validate_payable(self):
        self.ensure_one()
        if self._cn_iit_validate_settlement_configuration() != "payable":
            raise UserError(_("当前个人所得税档案不是应补缴税方向。"))
        run = self.cn_iit_reconciliation_run_id
        records = self._cn_iit_validate_payment_records()
        effective = records.filtered(
            lambda record: record.payment_status in {"succeeded", "reversed"}
        )
        if not effective.filtered(
            lambda record: record.payment_status == "succeeded"
        ):
            raise UserError(
                _("没有受控的个人所得税缴款成功记录，不能登记已付款。")
            )
        if (
            not run.has_payable_payment_difference
            or not run.currency_id.is_zero(run.payable_payment_difference)
        ):
            raise UserError(
                _("应补个人所得税与有效缴款本金尚未勾稽一致，不能标记已付款。")
            )
        ...
        if not evidence:
            raise UserError(
                _("登记个人所得税已缴款前必须具有已验证的正式缴款证据。")
            )
        return evidence

    def _cn_iit_validate_refund(self):
        self.ensure_one()
        self._require_manager()
        if self.state not in {"submitted", "accepted"}:
            raise UserError(
                _("只有已提交或已受理的个人所得税档案可以封存退库结果。")
            )
        if self.cn_payment_checksum:
            raise UserError(
                _("当前个人所得税退库结果已经封存，不能重复登记。")
            )
        if self._cn_iit_validate_settlement_configuration() != "refund":
            raise UserError(_("当前个人所得税档案不是应退税款方向。"))
        if (
            not self.cn_submission_checksum
            or self.cn_submission_integrity_state
            not in {"verified", "source_superseded"}
        ):
            raise UserError(
                _("请先完成并校验个人所得税正式申报封存。")
            )
        run = self.cn_iit_reconciliation_run_id
        records = self._cn_iit_validate_payment_records()
        refunded = records.filtered(
            lambda record: record.payment_status == "refunded"
        )
        if not refunded:
            raise UserError(
                _("没有受控的个人所得税已退库记录，不能封存退库结果。")
            )
        if (
            not run.has_refundable_refund_difference
            or not run.currency_id.is_zero(run.refundable_refund_difference)
        ):
            raise UserError(
                _("应退个人所得税与已退库本金尚未勾稽一致，不能封存退库结果。")
            )
        expected_date = max(refunded.mapped("payment_date"))
        if self.cn_iit_refund_date != expected_date:
            raise UserError(
                _(
        ...
        if len(refunded) == 1:
            ...
            if self.cn_iit_refund_reference != record.payment_reference:
                raise UserError(
                    _("退库参考号必须与唯一受控退库记录一致。")
                )
        elif not self.cn_iit_refund_reference:
            raise UserError(_("多笔退库记录需要填写受控汇总参考号。"))
        evidence = self._cn_verified_evidence(
            {"refund_receipt", "external_reference"}
        )
        if not evidence:
            raise UserError(
                _("封存个人所得税退库前必须具有已验证的正式退库回执证据。")
            )
        return evidence

    def action_ready(self):
        for filing in self.filtered("cn_iit_reconciliation_run_id"):
            filing._cn_iit_validate_obligation()
            filing._cn_iit_validate_settlement_configuration()
            filing._cn_iit_source_record(require_current=True)
        return super().action_ready()

    def action_submit(self):
        payloads = {}
        for filing in self.filtered("cn_iit_reconciliation_run_id"):
            record, evidence = filing._cn_iit_validate_submission()
            payloads[filing.id] = (
            ...
            self.env["sudo.compliance.audit.event"]._log_records(
                ...
                details={
                    "reconciliation_run_id": (
                        filing.cn_iit_reconciliation_run_id.id
                    ),
    ...
    def action_mark_paid(self):
        payloads = {}
        for filing in self.filtered("cn_iit_reconciliation_run_id"):
            evidence = filing._cn_iit_validate_payable()
            payloads[filing.id] = (
                filing._cn_payment_payload(evidence),
                evidence,
            )
    ...
    def action_seal_cn_iit_refund(self):
        payloads = {}
        for filing in self:
            evidence = filing._cn_iit_validate_refund()
            payloads[filing.id] = (
                filing._cn_payment_payload(evidence),
                evidence,
            )
    ...
    def _cn_submission_integrity(self):
        ...
        if not self.cn_submission_checksum:
            return "unsealed"
        try:
            record = self._cn_iit_source_record(require_current=False)
        ...
        if _checksum(payload) != self.cn_submission_checksum:
            return "changed"
        if (
            self.cn_iit_reconciliation_run_id.state == "superseded"
    ...
    def _cn_payment_integrity(self):
        ...
        try:
            ...
            if not evidence or any(item.state != "verified" for item in evidence):
                return "invalid"
            for item in self.cn_iit_payment_record_ids:
    ...
    def action_open_cn_iit_reconciliation(self):
        self.ensure_one()
        if not self.cn_iit_reconciliation_run_id:
            raise UserError(
                _("当前申报档案没有关联中国个人所得税勾稽结果。")
            )
        return {
            "type": "ir.actions.act_window",
            "name": _("个人所得税工资账表款勾稽"),
            "res_model": "sudo.cn.iit.period.reconciliation.run",
            "res_id": self.cn_iit_reconciliation_run_id.id,
            "view_mode": "form",
            "views": [
                (
                    self.env.ref(
...
class SudoChinaIitPeriodReconciliationRun(models.Model):
    ...
    def _compute_filing_archive_count(self):
        grouped = (
            self.env["sudo.compliance.filing"]._read_group(
                [("cn_iit_reconciliation_run_id", "in", self.ids)],
                ["cn_iit_reconciliation_run_id"],
                ["__count"],
            )
            if self
    ...
    def action_open_cn_filing_archive(self):
        ...
        if (
            filing
            and not filing.cn_iit_reconciliation_run_id
            ...
            filing.with_context(
                cn_iit_filing_archive_link=_CN_IIT_ARCHIVE_LINK_MARKER
            ).write({"cn_iit_reconciliation_run_id": self.id})
        action = self.env.ref(
            "sudo_global_finance.action_compliance_filings"
        ).sudo().read()[0]
        form_view = self.env.ref(
            "sudo_global_finance.view_compliance_filing_form"
        )
        action.update(
            {
                "view_mode": "form",
        ...
        if self.state != "succeeded":
            raise UserError(
                _("只有当前成功的个人所得税勾稽结果可以新建申报缴退税档案。")
            )
        filing_record = self.withholding_record_id
        ...
        payable = bool(
            self.has_filing_payable_amount
            and not self.currency_id.is_zero(self.filing_payable_amount)
        )
        refundable = bool(
            self.has_filing_refundable_amount
            and not self.currency_id.is_zero(self.filing_refundable_amount)
        )
        defaults = {
            "default_filing_name": _("个人所得税扣缴申报"),
            "default_profile_id": self.profile_id.id,
            "default_obligation_id": obligation.id or False,
            "default_filing_code": "CN-IIT-WHT",
            "default_filing_type": "cn_iit_withholding_return",
            ...
            "default_cn_iit_reconciliation_run_id": self.id,
        }
        if payable and not refundable:
            effective = self.payment_record_ids.filtered(
                lambda record: record.payment_status
                in {"succeeded", "reversed"}
            )
            if len(effective) == 1:
                defaults.update(
                    {
            ...
            if len(refunded) == 1:
                defaults.update(
                    {
                        "default_cn_iit_refund_date": refunded.payment_date,
                        "default_cn_iit_refund_reference": (
                            refunded.payment_reference
                        ),
                    }
                )
