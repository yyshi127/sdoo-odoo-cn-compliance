...
class TestChinaFormalComplianceReport(TransactionCase):
    ...
    def test_report_html_preserves_boundary_and_non_net_tax_impact(self):
        ...
        self.assertIn("Data basis snapshot".encode(), html)
        self.assertIn("External data state".encode(), html)
        ...
        self.assertIn(self.finding.evidence_required.encode(), html)
        self.assertIn("Source snapshot:".encode(), html)
        self.assertIn(self.finding.source_snapshot_json[0]["name"].encode(), html)
        self.assertIn(self.finding.source_snapshot_json[0]["status"].encode(), html)
        self.assertIn("Professional review snapshot:".encode(), html)
        self.assertIn(self.finding.professional_snapshot_json["state"].encode(), html)
