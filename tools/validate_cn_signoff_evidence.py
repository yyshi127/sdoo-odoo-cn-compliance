...
ACTION_EVIDENCE_REQUIREMENTS = {
    ...
    "representative_ux_walkthrough": (
        ...
        ("remediation tracker", ("remediation tracker", "remediation tracking", "remediation task", "整改跟踪", "整改任务")),
        ...
        ("report readiness", ("report readiness", "compliance report", "formal report", "报告就绪", "合规报告")),
        ("browser", ("browser", "浏览器")),
        ("desktop viewport", ("desktop viewport", "desktop resolution", "desktop screen", "桌面视口", "桌面分辨率")),
        ("laptop viewport", ("laptop viewport", "laptop resolution", "laptop screen", "smaller laptop", "小屏笔记本视口", "笔记本分辨率")),
...
def _validate(packet: dict[str, Any], evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    if evidence.get("preview_database") != packet.get("preview_database"):
        blockers.append("sign-off evidence preview database does not match the packet")
