...
PRODUCTION_SIGNOFF_BLOCKER_ACTIONS = {
    ...
    "current official sources and released rules require professional sign-off evidence": [
        ...
        },
        {
            ...
            "required_evidence": "Current official-source monitoring results, latest monitor run state/completion time, result integrity state, 64-character SHA-256 result checksum, changed or failed run disposition and any local jurisdiction updates reviewed for the target period.",
    ...
    "customer-specific data gaps, evidence gaps and open critical risks must be reviewed": [
        {
            ...
            "required_evidence": (
                "Customer-specific target periods, posted accounting data, external "
                "invoice/filing/payment/payroll datasets, acquisition or authorization "
                "basis, evidence gaps, open high/critical risks, remediation status, "
                "customer-visible limitation statement and controlled AI limitations reviewed."
            ),
...
def _status(
    *,
    ...
    signoff_candidate_selector_tool = {
        "path": SIGNOFF_CANDIDATE_SELECTOR_TOOL_PATH.as_posix(),
        "included_in_manifest": _manifest_includes(
    ...
    if real_data_closed_loop:
        ...
        real_data_closed_loop_summary = {
            ...
            "ux_view_clarity_contracts": real_data_closed_loop.get(
                "ux_view_clarity_contracts"
            ),
            "menu_action_contracts": real_data_closed_loop.get(
                "menu_action_contracts"
            ),
            "workbench_action_contracts": real_data_closed_loop.get(
...
def _write_markdown(status: dict[str, object], path: Path) -> None:
    ...
    lines = [
        ...
        f"- UX view clarity contract evidence ready: `{real_data_readiness.get('has_ux_view_clarity_contract_evidence', False)}`",
        f"- Menu/action contract evidence ready: `{real_data_readiness.get('has_menu_action_contract_evidence', False)}`",
        f"- Workbench action contract evidence ready: `{real_data_readiness.get('has_workbench_action_contract_evidence', False)}`",
        ...
        f"- Customer scope/gap review evidence ready: `{real_data_readiness.get('has_customer_scope_gap_review_evidence', False)}`",
        f"- Customer data-scope evidence ready: `{real_data_readiness.get('has_customer_data_scope_review_evidence', False)}`",
        f"- Customer evidence-gap evidence ready: `{real_data_readiness.get('has_customer_evidence_gap_review_evidence', False)}`",
